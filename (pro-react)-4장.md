...
리액트 Dnd는 모니터라고 하는 내부 상태 저장소에 대한 래퍼를 통해 컴퍼넌트에 이러한 상태를 노출 시킨다. 모니터는 드래그 앤드 드랍 상태 변화에 대한 응답을 컴퍼넌트의 프로퍼티로 업데이트 하게 해준다.  
...
어떻게 그런 props이 컴포넌트에 인젝트 되는 것을 설정할까? 어떻게 드래그 드랍 이벤트의 반응 상에서 사이드 이펙트가 수행되나?Drag source, drop target, 리액트 Dnd의 주요 추상 유닛을 보자. 그것들은 실제로 컴포넌트에 타입, 아이템, 사이드 이펙트, 콜렉팅 함수들이 모두 역여있다.  
  
컴포넌트를 생성하거나 드래그 가능한 어떠한 부분을 만들 때 마다 drag source로서 컴포넌트를 둘러쌀 필요가 있다. 모든 drag source는 특정 타입으로서 등록 되며, 컴포넌트의 props로 부터 아이템을 생상하는 메서드를 구현하여야 한다. 또한 부가적으로 드래그와 드랍 이벤트를 다루는 몇몇 메서드를 지정할 수 있다.  
  
drop target은 drag source와 매우 유사하며, 차이라면 한개의 drop target은 한번에 여러  아이템으로 등록 될 수 있다는 것이다. 그리고, 아이템을 만들어 내는 대신에 hover 또는 drop을 다룬다.

### 고차 컴포넌트와 es7 decorators
어떻게 컴포넌트를 둘러 싸며, 그것의 의미는 무엇인가? 이런한 고차 컴포넌트에 익숙하지 않다면 이 글을 참조하라.   
  
고차 컴포넌트는 단지 리액트 컴포넌트 클래스를 가지는 펑션이고, 또 다른 리액트 컴포넌트를 반환하는 펑션이다. 그것은 당신이 정의한 컴포넌트의 기능들을 제공하며 몇몇 유용한 동작을 추가한다.  
  
리액트 Dnd에서 DragSource와 DropTarget은 최고 수준의 export된 함수일 뿐 아니라 고차 컴포넌트이다.
...
### 사양 객체
컴포넌트가 드래그 앤드 드롭 이벤트에 '반응' 하는 방법을 지정  
  
### 콜렉팅 함수  
컴포넌트에 인젝트할 프로퍼티들의 일반 객체를 반환하여야 한다. 두가지 파라미터를 받으며, 하나는 모니터 하나는 커넥터이다. 리엑트 컴포넌트와의 모든 커뮤니케이션은 props를 통해 발생하고, 그래서 리엑트 Dnd가 특별 props를 컴포넌트로 주입하는 것은 타당하다.  
  
콜렉팅 함수는 노드를 Dnd 백엔드로 연결할 수 있게 하며, 모니터를 통해 드래그 정보를 수집할 수 잇게 한다.
